...
const Login = ({ onLoginResult }) => {
  ...
  const handleLogin = async () => {
    try {
      const response = await axios.post('https://studio-backend-sigma.vercel.app/user/login', { username, password }, { withCredentials: true });
      ...
      <div className=' w-11/12 max-w-[700px] px-10 py-20 rounded-3xl bg-white border-2 border-gray-100'>
        ...
        <div className='mt-8'>
          ...
          <div className='mt-8 flex flex-col gap-y-4'>
            <button onClick={handleLogin} className='active:scale-[.98] active:duration-75 transition-all hover:scale-[1.01]  ease-in-out transform py-4 bg-violet-500 rounded-xl text-white font-bold text-lg'
            >Login</button>
            {/* <button 
