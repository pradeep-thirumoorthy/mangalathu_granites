...
const SalesExecutiveDashboard = () => {
    ...
    const fetchSummary = async () => {
      ...
      try {
        ...
        const res = await axios.get(
          `http://localhost:3010/dashboard/summary/salesExe/${userId}`,
          { params: { month, year } }
        );
  ...
  return (
    <div style={{ padding: 24 }}>
      <Title level={3} style={{ marginBottom: 24 }}>Sales Executive Dashboard</Title>

      {/* TOP ROW: QR Code, Personal Details, Today's Attendance */}
      <Row gutter={[20, 20]} align="stretch">
        ...
        <Col xs={24} sm={24} md={8} style={{ display: "flex" }}>
          <Card title="Your Details" style={{ flex: 1 }}>
            ...
            <DetailItem label="Commission Rate:" value={`${user.commissionRate || 0}%`} />
          </Card>
        </Col>
      ...
      <Row gutter={[20, 20]} style={{ marginTop: 16 }}>
        ...
        <Col xs={24} md={8}>
          <Card title="Payroll (This Month)" style={cardStyle}>
            {payroll ? (
              <>
                <DetailItem label="Base Salary:" value={`₹${payroll.financials.baseSalary}`} />
                <DetailItem label="Commission :" value={`₹${payroll.financials.commissionEarned}`} />
                <DetailItem label="Net Salary:" value={`₹${payroll.financials.netSalary}`} />
